...
4. **After successfully configuring the WLAN, the sensor closes the network and should inform you about its address in the configured WLAN. If the sensor does not display the IP address, it can be looked up in the router.**
...
<CardGrid>
	<Card title="The wrong way around?" icon="information">
    		Try turning the sensor 180 degrees. Otherwise, a check can be carried out to see whether numbers are displayed via the address assigned during configuration. If only zeros are displayed, the sensor must be rotated. If only zeros are displayed at different positions or no numbers appear, please contact the service provider.
  	</Card>
